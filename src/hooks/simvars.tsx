...
interface SimVarContextType {
  ...
  update: (
    name: string,
    unit: SimVarUnitsEnum,
    value: SimVarValue | SimVarSetter,
    proxy?: string
  ) => void;
  register: (
    name: string,
...
export const SimVarProvider = (props: { children?: any }) => {
  const [listeners, setListeners] = SolidJS.createSignal<
    Record<string, number[]>
  >({});
  const [cache, setCache] = SolidJS.createSignal<
    Record<string, { value: SimVarValue; lastUpdated: number }>
  >({});
  ...
  const startUpdateLoop = () => {
    updateInterval = setInterval(() => {
      ...
      const stateUpdates: Record<
        string,
        { value?: SimVarValue; lastUpdated: number }
      > = {};
      ...
      for (const [key, intervals] of Object.entries(currentListeners)) {
        ...
        const lastUpdated =
          (currentCache[key] ? currentCache[key].lastUpdated || 0 : 0) +
          deltaTime;
  ...
  return (
    <SimVarContext.Provider value={contextValue}>
      {props.children}
...
export const useSimVar = (
  ...
  SolidJS.Accessor<SimVarValue>,
  (newValue: SimVarValue | SimVarSetter) => void
] => {
  const contextValue = SolidJS.useContext(SimVarContext);
  const [value, setValue] = SolidJS.createSignal(
    contextValue.retrieve(name, unit, false, 0)
  );
...
export const useGlobalVar = (
  ...
  const contextValue = SolidJS.useContext(SimVarContext);
  const [value, setValue] = SolidJS.createSignal(
    contextValue.retrieve(name, unit, false, 1)
  );
